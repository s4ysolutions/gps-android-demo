...
class AndroidGPSUpdatesProvider : IGPSUpdatesProvider {
    ...
    override val status = object : IGPSProvider.IStatus {
    ...
    private val locationUpdatesListener =
        LocationListener { location ->
            val update = GPSUpdate(location)
            _updates.tryEmit(update)
        }

    private var gpsHandlerThread: HandlerThread? = null
    @RequiresPermission(anyOf = [Manifest.permission.ACCESS_FINE_LOCATION, Manifest.permission.ACCESS_COARSE_LOCATION])
    override fun startUpdates(context: Context, looper: Looper?) {
        stopUpdates()
